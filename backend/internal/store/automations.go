...
type automationRow struct {
	...
	TriggerReentry      bool      `db:"trigger_reentry"`
	TriggerRespectOptOut bool     `db:"trigger_respect_opt_out"`
	Steps               []byte    `db:"steps"`
...
func (r automationRow) toAutomation() *Automation {
	return &Automation{
		...
		TriggerReentry:       r.TriggerReentry,
		TriggerRespectOptOut: r.TriggerRespectOptOut,
		Steps:                r.Steps,
		Status:               r.Status,
...
func (s *Store) CreateAutomation(ctx context.Context, a *Automation) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO automations (id, workspace_id, name, description, trigger_type, trigger_label,
			trigger_conditions, trigger_delay, trigger_reentry, trigger_respect_opt_out, steps, status)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.WorkspaceID, a.Name, a.Description, a.TriggerType, a.TriggerLabel,
		marshalJSON(a.TriggerConditions), a.TriggerDelay, a.TriggerReentry, a.TriggerRespectOptOut,
		a.Steps, a.Status)
	return err
}
...
func (s *Store) UpdateAutomation(ctx context.Context, a *Automation) error {
	_, err := s.db.ExecContext(ctx, `
		...
		marshalJSON(a.TriggerConditions), a.TriggerDelay, a.TriggerReentry, a.TriggerRespectOptOut,
		a.Steps, a.Status, a.WorkspaceID, a.ID)
	return err
}
